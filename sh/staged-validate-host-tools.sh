...
if [ ! -x staging_dir/host/bin/apk ]; then
  echo "Missing staging_dir/host/bin/apk; rebuilding OpenWrt apk host tool"
  make package/system/apk/host/clean V=s || true
  make package/system/apk/host/compile V=s || yes_make package/system/apk/host/compile V=s
fi
if [ ! -x staging_dir/host/bin/apk ]; then
  echo "ERROR: staging_dir/host/bin/apk is still missing after rebuild"
  find staging_dir/host build_dir/hostpkg -maxdepth 4 \( -iname 'apk' -o -iname '*apk*' \) || true
  exit 1
fi
...
ls -l staging_dir/host/bin/apk || true
